...
function imageGene(gene, selector) {
	...
	var map = {
		HGT: {start: 1, len: 2, callback: function(FF){
			binding.cssTranscription["height"] = parseFloat(parseInt(FF,16)) / parseFloat(parseInt("FF",16)) + "%";
		}},
		WID: {start: 3, len: 2, callback: function(FF){
			binding.cssTranscription["width"] = parseFloat(parseInt(FF,16)) / parseFloat(parseInt("FF",16)) + "%";
		}},
